/**
 * API handler for income management
 * 
 * This module provides endpoints for creating and retrieving income records,
...
/**
 * Handle GET request to fetch income records with pagination and filters
 * 
...
async function getIngresos(req, res) {
  ...
  // Build query parts
  let countQuery = 'SELECT COUNT(*) FROM ingresos WHERE user_id = $1';
  let query = 'SELECT * FROM ingresos WHERE user_id = $1';
  ...
  // Add sorting
  const validSortFields = ['fecha', 'monto', 'descripcion', 'id'];
...
/**
 * Handle POST request to create a new income record
 * 
 * @param {Object} req - The request object
 * @param {Object} res - The response object
 */
async function createIngreso(req, res) {
  ...
  if (!validation.isValid) {
    throw new ValidationError('Datos de ingreso inválidos', validation.errors);
  }
  
  // Insert into database
  const result = await db.query(
    'INSERT INTO ingresos (monto, descripcion, fecha, categoria, user_id) VALUES ($1, $2, $3, $4, $5) RETURNING id',
...
/**
 * Handle GET request for a single income record by ID
 * 
 * @param {Object} req - The request object
 * @param {Object} res - The response object
 */
async function getIngresoById(req, res) {
  ...
  // Validate ID
  if (!id) {
    throw new BadRequestError('ID de ingreso no proporcionado');
  }
  ...
  return res.status(200).json(
    formatSuccess(
      result.rows[0],
      'Ingreso recuperado exitosamente'
    )
  );
}

/**
 * Handle PUT request to update an income record
 * 
 * @param {Object} req - The request object
 * @param {Object} res - The response object
 */
...
/**
 * API handler for individual income records by ID
 * Supports GET, PUT, DELETE operations on specific records
 * 
 * @type {function} 
 */
export const handleIngresoById = registerRoutes(
  {
    GET: getIngresoById,
    PUT: updateIngreso,
...
/**
 * Default API handler for income collection
